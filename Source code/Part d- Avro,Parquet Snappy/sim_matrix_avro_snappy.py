...
conf=SparkConf().setAppName("HW3_Part3_Avro_Snappy")
sc=SparkContext(conf=conf)
sqlContext=SQLContext(sc)
sqlContext.setConf("spark.sql.avro.compression.codec","snappy")

#Read the inverted index as dataframe and convert it into rdd
inv_index_data_avro=sqlContext.read.format("com.databricks.spark.avro").load(sys.argv[1])
...
sim_matrix_df.write.format("com.databricks.spark.avro").save(sys.argv[2])
